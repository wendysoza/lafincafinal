...
function mostrar(id_cli)
{
	$.post("../ajax/clientes.php?op=mostrar",{id_cli : id_cli}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);

        $("#id_cli").val(data.id_cli);
        $("#nombres_cli").val(data.nombres_cli);
        $("#cedula_cli").val(data.cedula_cli);
        $("#telefono_cli").val(data.telefono_cli);
        $("#direccion_cli").val(data.direccion_cli);
        $("#email_cli").val(data.email_cli);
 	});
}

//Función para desactivar registros
function desactivar(id_cli)
{
	bootbox.confirm("¿Está seguro de desactivar el cliente?", function(result){
		if(result)
        {
        	$.post("../ajax/clientes.php?op=desactivar", {id_cli : id_cli}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
...
function activar(id_cli)
{
	bootbox.confirm("¿Está seguro de activar el cliente?", function(result){
		if(result)
        {
        	$.post("../ajax/clientes.php?op=activar", {id_cli : id_cli}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});	
        }
